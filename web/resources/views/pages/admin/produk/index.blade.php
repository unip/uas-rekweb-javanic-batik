...
@section('content')
    <div class="flex items-center border-b border-b-gray-300 pb-5 mb-5">
        ...
        <a href="{{ route('admin.products.create') }}" class="btn-primary ml-auto">+
            Produk</a>
    </div>
    ...
    <div x-data="{ deleteConfirm: false, id: null, name: null, action: null }" class="table-kategori h-full flex">
        @if ($products->count() === 0)
            <div class="maaf bg-white p-5 flex-1 flex flex-col items-center justify-center">
                ...
                <p class="mb-5">Tambah produk yuk</p>
                            ...
                            <tr class="border-b border-b-slate-300 hover:bg-slate-200 transition duration-200">
                                ...
                                <td class="p-3">
                                    <button class="bg-gray-700 text-white p-3 hover:bg-gray-900"
                                        @click="deleteConfirm = true; id = `{{ $item['id'] }}`; name = `{{ $item['nama_produk'] }}`; action = `{{ route('admin.products.destroy', ['product' => $item['id']]) }}`">delete</button>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
